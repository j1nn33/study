...
for IP in DEVICES_IP:
    ...
    with telnetlib.Telnet(IP) as t:         # класс Telnet представляет соединение к серверу
        ...
        t.read_until(b'Username:')          # read_until -  Указывает, до какой строки следует считывать вывод
        ...
        output = t.read_very_eager().decode('utf-8')    # read_very_eager - считать всё, что получается
        print(output)
