# Есть словарь с информацией о разных устройствах.
# Нужно запросить у пользователя ввод имени устройства (r1, r2 или sw1). И
# вывести информацию о соответствующем устройстве на стандартный поток вывода
# (информация будет в виде словаря).
#
# Пример выполнения скрипта (ключи могут быть в другом порядке):
# $ python task_4_2.py
# Enter device name: r1
# {'ios': '15.4', 'model': '4451', 'vendor': 'Cisco', 'location': '21 New Globe Walk', '
# ip': '10.255.0.1'}
#
##############################################
"""
 Задание словаря
 Ввод информации от пользователя
 Обработка информации
 Выыод информации
"""
                                # Задание словаря (в словаре в качестве значения можно используется словарь)
london_co = {
    'r1' : {
    'location': '21 New Globe Walk',
    'vendor': 'Cisco',
    'model': '4451',
    'ios': '15.4',
    'ip': '10.255.0.1'
    },
    'r2' : {
    'location': '21 New Globe Walk',
    'vendor': 'Cisco',
    'model': '4451',
    'ios': '15.4',
    'ip': '10.255.0.2'
    },
    'sw1' : {
    'location': '21 New Globe Walk',
    'vendor': 'Cisco',
    'model': '3850',
    'ios': '3.6.XE',
    'ip': '10.255.0.101',
    'vlans': '10,20,30',
    'routing': True
    }
}

                                # Ввод информации от пользователя
name = input('Введите имя устройства ')
#name = 'sw1'
print(name)

                                # Обработка информации

                                # Выыод информации
print('Информация со словаря по ', name)
print (london_co[name])