...
'''
Задание 11.5 на основе задания 11.3
 ...
 *  last_active и в нем должна находиться строка, в формате: YYYY-MM-DD HH:MM:SS
...
def update_info(db_filename):
    ...
    for row in data:
        row=list(row)
        row.insert(-2,0)     # замена active на 0
        row.pop(-2)
        row=tuple(row)
    ...
    for row in old_data:
        try:
            with conn:
                query1 = '''REPLACE INTO dhcp (mac, ip, vlan, interface, switch, active, last_active)
                            values (?, ?, ?, ?, ?, ?, ?)'''
...
def delete_old_data(db_filename):
    conn = sqlite3.connect(db_filename)
    data = []
    query = 'SELECT * from dhcp'
    data = conn.execute(query)
    ...
    day_ago = now - timedelta(days = 0.8)
    print (day_ago)
   
    for row in data:

        if str(day_ago)>row[6]:
            print ('removing old data' , row)
            try:
                with conn:
                    print ('selected', row[6])
                    conn.execute("delete from dhcp where mac='%s' " % row[0])
                    print ('deleting',row)
            except sqlite3.IntegrityError as e:
                print('Error occured: ', e)
...
def ins_dhcp(dhcp_file):
    ...
    for row in result:
        ...
        row.append(1)
        now_data = str(datetime.today().replace(microsecond=0))
        row.append(now_data)
        row=tuple(row)
        print (row)
...
if __name__ == "__main__":
    ...
    if not db_exists:
        ...
        # закоментирован кусок кода обноления базы данных актуальной информацией
...
##################################################################################        
    """
    $ sqlite3 /home/ubuntu/workspace/python/tasks/11_sql/11_5/dhcp_snooping.db
