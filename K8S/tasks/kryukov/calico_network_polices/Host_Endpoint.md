...
###  Failsafe rules
```
Как только вы определяете HostEndpoint для интерфейса, автоматически к этому
...
```
Эту защиту можно отключить, изменить или расширить. Это делается при помощи параметров
...
metadata:
  ...
  labels:      # Важно тк дальнейшее обращение будет идти через labels, а не name: <name of endpoint>
    role: webserver
    environment: production
spec:
  interfaceName: eth0
  node: <node name or hostname>
  expectedIPs: ["10.0.0.1"]   # Массив ip адресов 
```

Пример описания HEP для контрол ноды моего кластера:
...
spec:
  ...
  node: control1.kube.local
  expectedIPs:
    - 192.168.1.171
```
Все HEP для кластера описаны в файле ./K8S/tasks/kryukov/calico_network_polices/network_polices/cluster-heps.yaml

## Firewall
```
как при помощи HEP и GlobalNetworkPolicy организовать простейший firewall для кластера.
./K8S/tasks/kryukov/calico_network_polices/network_polices/image/scheme.jpg

# у хоста 2 интерфейса  eth0 eth1
# таблица маршрутищзации Kernel routing table
# Pod (Workload-1 Workload-2)
# у Pod свой виртуальный сетевой интерфейс caliXXX
# Если пакет предназначен для хостового ППО (ssh), то он пойдет на Local service (правила preDNAT true) см ниже 
# applyOnForward если пакет идет транзитом (на под или в другую сеть) на другой сетевой интерфейс (eth1, caliXXX)
```
##### preDNAT и applyOnForward
```
# Для фильтрации трафика нам нужно ловить пакеты:
#      - на входе сетевого интерфейса хоста.  до того как к нему будут применены действия по замене IP и портов. Т.е. перед DNAT.
...
```
Схема кластера 
./K8S/k8s_install_kubeadm/images/cluster.jpg

- NAT не делаем (тк calico Firewall несколько ограничен)
- Разрешить весь входящий и исходящий трафик:
...
- В кластере используются два кластерных IP: 
    - 192.168.1.180 - разрешить подключаться на tcp/80 и tcp/443
    - 192.168.1.189 - разрешить подключаться на tcp/7443. *По хорошему этот порт надо добавить в исключения в kind: FelixConfiguration
- Разрешить входящий трафик только на определённые сервисы типа NodePort. 
```
### Разрешить весь входящий и исходящий трафик
...
metadata:
  name: default-rules
spec:         # Определяется для всех нод кластера - задавали в ./K8S/tasks/kryukov/calico_network_polices/network_polices/cluster-heps.yaml labels: node-endpoint: home
  selector: node-endpoint == 'home'
  order: 20   # номер последовательности в цепочки iptables (чем ниже, тем выше приоритет те стоит в верху списка)
  preDNAT: true
  applyOnForward: true
  types:      # на входящий трафик
    - Ingress
  ingress:
    - action: Allow
      source:
        nets:
          - 10.233.0.0/16
    - action: Deny # политика по умолчанию 
```
```
- selector: node-endpoint == 'home' - тут мы указываем labels, которые ставили на HostEnpoint. Т.е. политика
...
- types: [ Ingress ] - в политике будут описаны только правила Ingress.
   ...
   - action: Deny - Явно запрещаем все остальные пакеты.
    при включении preDNAT: true нам явно надо писать правило Deny All в конце.
...
./K8S/tasks/kryukov/calico_network_polices/network_polices/cluster-heps.yaml
```
```yaml
apiVersion: projectcalico.org/v3
kind: HostEndpoint
...
#### Вариант 2 с использовением labal home
  ...
  ingress:
    - action: Allow
      source:
        nets:
          - 10.233.0.0/16
    - action: Allow             # Для 2 Варианта с использовением labal home
      source:
        selector: node-endpoint == 'home'
    - action: Deny
```
...
### Кластерные IP адреса.
```
B кластере есть MetalLB и кластерный IP используемый в сервисе типа LoadBalancer ingress контроллера -
192.168.1.180. На этом IP необходимо открыть порты 80 и 443.

Для доступа к kubernetes API через HA Proxy я использую IP 192.168.1.189 и порт 7443.

Эти IP адреса не указаны ни в одном HostEndpoint тк они плавающие 
